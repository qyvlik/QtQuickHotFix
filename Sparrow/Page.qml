...
Rectangle {
    ...
    property StackView stackView: pageStackWindow != null ? pageStackWindow.stackView : null
    ...
    function __PushPage(url, model, properties, callback, errorCallback){

        model = model || Component.PreferSynchronous;

        var component = Qt.createComponent(url, model);
        ...
        try {
            if(component.status !== Component.Ready
                    && model === Component.Asynchronous) {

                // console.log("will load hot fix source:", url);

                component.onStatusChanged.connect(function() {
                    if(component.status === Component.Ready) {
                        component.onStatusChanged.disconnect(arguments.callee);
                        pushBlock();
